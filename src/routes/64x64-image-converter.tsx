...
const TITLE = '64x64 Image Converter - Resize Images';
const DESCRIPTION =
  '64x64 image converter for local JPG, PNG, and WebP resizing. Create favicons, small icons, sprites, and compact avatars.';
...
function ConverterPage() {
  ...
  async function downloadZip() {
    ...
    downloadBlob(blob, '64x64-image-converter-outputs.zip');
  }
        ...
        <section className="space-y-8">
          <div className="mx-auto flex max-w-4xl flex-col items-center pt-4 text-center">
            ...
            <p className="mx-auto mt-5 max-w-4xl text-center text-lg leading-8 text-slate-700">
              Use this 64x64 image converter when a favicon, small icon, sprite,
              chat avatar, UI control, or compact product badge asks for an
              exact 64 x 64 pixel file. The converter keeps the work local in
              your browser, and the 64x64 image converter gives you a live
              square preview while you choose whether the whole image should be
              preserved with padding, filled by a center crop, or resized
              exactly.
            </p>
            <div className="mx-auto mt-6 grid w-full max-w-4xl gap-3 sm:grid-cols-3">
              {[
                ['64 x 64', 'Locked pixel canvas'],
                ['Local', '64x64 image converter stays local'],
      ...
      <div className="rounded-md border border-dashed border-slate-300 bg-white/70 p-10 text-center">
        <div className="mx-auto flex max-w-2xl flex-col items-center">
          ...
          <h2 className="mt-6 text-2xl font-black">
            Upload images for 64 x 64 output
          </h2>
          <p className="mt-3 text-slate-600">
            Add JPG, PNG, WebP, GIF, SVG, or BMP files. This 64x64 image
            converter reads dimensions locally, creates a square preview, and
            exports exact 64 x 64 files without sending source images away. The
            64x64 image converter is designed for quick square outputs, not a
            long manual setup.
          </p>
...
function ConverterWorkspace({
  ...
  return (
    <div className="min-w-0 space-y-6">
      ...
      <Card className="premium-card min-w-0">
        <CardHeader>
          <CardTitle>Generate 64 x 64 files</CardTitle>
          <CardDescription>
            The 64x64 image converter output is locked to 64 x 64 px. Current
            mode: {resizeModeLabel(settings.fit)}.
          </CardDescription>
        </CardHeader>
  ...
  return (
    <Card className="premium-card min-w-0">
      <CardHeader>
        ...
        <CardDescription>
          The 64x64 image converter canvas is fixed at 64 x 64 px. Choose how
          the source should fit the square.
        </CardDescription>
      </CardHeader>
    ...
    <div className="min-w-0 space-y-4">
      ...
      <div className="relative h-80 min-w-0 overflow-hidden rounded-lg border border-slate-200 bg-slate-950/[0.03] p-5">
        ...
        >
          <canvas
            ref={canvasRef}
            aria-label={`64 x 64 preview for ${asset.name}`}
            className={cn(
  ...
  return (
    <Card className="premium-card">
      <CardHeader>
        ...
        <CardDescription>
          {outputs.length
            ? 'Download exact 64 x 64 files individually or as one ZIP.'
            ...
            {outputs.length ? (
              ...
              <TableRow>
                <TableCell colSpan={5} className="text-slate-500">
                  Upload images, choose a square mode, and let the 64x64 image
    ...
    <section className="space-y-12 border-t border-slate-200/70 pt-12">
      <div className="grid gap-8 lg:grid-cols-[0.85fr_1.15fr] lg:items-start">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black tracking-normal text-slate-950">
            A 64x64 image converter solves a specific publishing problem
          </h2>
        </div>
        <div className="space-y-4 text-base leading-8 text-slate-700">
          <p>
            A 64x64 image converter changes a source image into an exact 64 x 64
            pixel file. That sounds simple, but the useful part is the decision
            it forces: should the whole source stay visible, should the square
            be filled by cropping the center, or should the full image be
            stretched to the requested size? A practical 64x64 image converter
            ...
            padding where a platform expected a filled square. The 64x64 image
            converter makes that tradeoff visible before the file is saved.
          </p>
          <p>
            This 64x64 image converter is built for the exact cases where a
            general resizer feels slower than it should. Instead of entering
            width and height every time, the 64x64 image converter locks the
            canvas, keeps the preview square, and lets you focus on format,
            quality, background, and fit mode. The result is a repeatable 64x64
            image converter workflow for favicons, tiny avatars, sprite tiles,
            compact badges, toolbar icons, product chips, and any small square
            image that needs predictable pixel dimensions.
          </p>
        </div>
      </div>
        ...
        {[
          {
            ...
            text: 'Every output from this 64x64 image converter is locked to 64 x 64 pixels, so there is no accidental 63 px or 65 px export.',
          },
          {
            title: 'Whole-image option',
            text: 'The 64x64 image converter uses padding when small logos, badges, or pixel-style artwork must remain fully visible inside the square.',
          },
          {
            title: 'Fill-square option',
            text: 'The 64x64 image converter can use center crop when a tiny avatar, favicon source, sprite tile, or UI thumbnail needs a clean edge-to-edge square.',
          },
          {
            title: 'Local batch export',
            text: 'The 64x64 image converter reads files in the browser, supports multiple images, and prepares individual downloads or a ZIP.',
          },
        ].map((item) => (
        ...
        <div className="grid gap-8 lg:grid-cols-[0.9fr_1.1fr]">
          ...
          <div className="space-y-4 text-sm leading-7 text-slate-300">
            <p>
              ...
              text until it becomes hard to read. A 64x64 image converter helps
              you deliver the final size yourself, so the uploaded file is
              already aligned with the placement and less likely to be changed
              by an automated platform pipeline. A 64x64 image converter reduces
              that risk by making the final square the file you export, not a
              surprise created after upload.
            </p>
            <p>
              ...
              unplanned. With a 64x64 image converter, each image starts from
      ...
      <div className="grid gap-8 lg:grid-cols-[1.05fr_0.95fr]">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black tracking-normal text-slate-950">
            Where a 64x64 image converter is useful
          </h2>
          <p className="mt-4 text-base leading-8 text-slate-700">
            A 64x64 image converter is most valuable when the size is not a
        ...
        <div className="grid gap-3">
          {[
            [
              'Favicons and browser UI assets',
              'Use the 64x64 image converter when a website, extension, or internal dashboard needs a compact square icon source.',
            ],
            [
              'Small app icons and UI controls',
              'Prepare toolbar icons, menu images, settings icons, notification images, and product badges without guessing the final pixel size.',
            ],
            [
              'Sprite sheets and game assets',
              'Convert character heads, inventory icons, collectible badges, and pixel-style tiles into consistent 64 x 64 source squares.',
            ],
            [
              'Compact avatars and directories',
              'The 64x64 image converter can create small profile images for chat lists, admin tables, comments, and dense account interfaces.',
            ],
          ].map(([title, text]) => (
            <div
              key={title}
      ...
      <div className="bg-white/75 p-6 ring-1 ring-slate-200">
        ...
        <h2 className="mt-3 text-3xl font-black tracking-normal text-slate-950">
          The best 64x64 image converter workflow is deliberate
        </h2>
        <ol className="mt-6 grid gap-4 lg:grid-cols-4">
          {[
            {
              title: 'Upload once',
              text: 'Add one image or a batch. The 64x64 image converter keeps thumbnails visible so you can move between files without returning to the upload screen.',
            },
            {
              title: 'Choose the square mode',
              text: 'Use the 64x64 image converter to fit with padding for preservation, fill by center crop for a full thumbnail, or resize exactly when distortion is acceptable.',
            },
            {
              title: 'Check the preview',
              text: 'The 64x64 image converter shows the real output frame, so you can see padding, cropping, and background color before export.',
            },
            {
              title: 'Export the batch',
              text: 'The 64x64 image converter can process one selected image or the entire queue, then download each 64 x 64 file or collect every result in a ZIP archive.',
            },
          ].map((step, index) => (
        ...
        <div className="premium-card p-6">
          ...
          <div className="mt-4 space-y-4 text-sm leading-7 text-slate-600">
            <p>
              ...
              Print targets. This 64x64 image converter is the focused page for
              favicon, small icon, sprite, badge, and compact avatar
              requirements, using the same browser-based canvas resizing engine
              but removing unnecessary width and height decisions.
            </p>
            <p>
              The 64x64 image converter value is practical: it gives users a
              ...
              and download clean files without creating an account. A 64x64
        ...
        <div className="premium-card p-6">
          ...
          <div className="mt-4 space-y-4 text-sm leading-7 text-slate-600">
            <p>
              ...
              stretched. The 64x64 image converter still offers preservation
              with padding when missing edges would be worse than a stretched
              square.
            </p>
            <p>
              A 64x64 image converter should not hide that tradeoff. This 64x64
              image converter labels each mode plainly, updates the preview as
              settings change, and keeps the final canvas fixed. That makes the
              64x64 image converter a real converter rather than a guessing
              step. You can see whether the output is acceptable before any
              download is created.
            </p>
          </div>
        </div>
      ...
      <div className="grid gap-8 lg:grid-cols-[0.8fr_1.2fr]">
        <div>
          ...
          <h2 className="mt-3 text-3xl font-black tracking-normal text-slate-950">
            64x64 image converter questions
          </h2>
        </div>
        <div className="grid gap-3">
          {[
            [
              'Does the 64x64 image converter upload my images?',
              'No. The 64x64 image converter reads the files in your browser and creates outputs locally. That is useful for private photos, unreleased products, client images, internal documents, and any file that should not be sent to a remote image service.',
            ],
            [
              'Will the output always be exactly 64 x 64 pixels?',
              'Yes. The 64x64 image converter canvas is locked, and every converted file is exported at 64 x 64 pixels. The mode only changes how the source image is placed inside that square.',
            ],
            [
              'Should I use padding or center crop?',
              'Use padding when the entire source must stay visible. Use center crop when the subject is central and the square should be filled edge to edge. The 64x64 image converter preview makes this decision visible before download.',
            ],
            [
              'Can I convert many images at once?',
              'Yes. The 64x64 image converter can process a batch with the same settings, convert all images, and download the results individually or in one ZIP. Batch export is helpful for icon sets, sprite assets, badge packs, dense directories, and CMS media cleanup.',
            ],
            [
              'Will a 64x64 image converter improve a tiny source image?',
              'It can enlarge a small file to 64 x 64 pixels, but it cannot create detail that was not present in the original. For sharper output, start with a source image that is at least 64 pixels wide and 64 pixels tall.',
            ],
            [
              'When should I use the general Image Resizer instead?',
              'Use the general Image Resizer when you need other targets such as Open Graph images, story creatives, print sizes, or custom dimensions. Use this 64x64 image converter when the exact square size is the requirement.',
            ],
          ].map(([question, answer]) => (
...
function outputName(sourceName: string, format: ResizeSettings['format']) {
  ...
  return `${base}-64x64.${extensionForType(format)}`;
}
...
export const Route = createFileRoute('/64x64-image-converter')({
  head: () => ({
    meta: [{ title: TITLE }, { name: 'description', content: DESCRIPTION }],
    links: [
      {
        rel: 'canonical',
        href: `${envConfigs.app_url}/64x64-image-converter`,
      },
    ],
  }),
